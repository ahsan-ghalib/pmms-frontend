...
export function VendorDashboard() {
  ...
  const [activeFilters, setActiveFilters] = useState({ period: "30", months: "6", branchId: "" });
  
  const handleApplyFilters = () => {
    setActiveFilters({ period, months, branchId });
  };
  ...
  const handleResetFilters = () => {
    ...
    setMonths("6");
    setBranchId("");
    setActiveFilters({ period: "30", months: "6", branchId: "" });
  };
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      setError(null);
      const params = { period: activeFilters.period, months: activeFilters.months };
      if (activeFilters.branchId) params.branch_id = activeFilters.branchId;
  ...
  useEffect(() => {
    fetchDashboardData();
  }, [dateRange.startDate, dateRange.endDate, activeFilters]);
  ...
  const dailyOrderChartOptions = {
    chart: { type: "area", height: 320, toolbar: { show: false }, zoom: { enabled: false }, fontFamily: "inherit" },
    dataLabels: { enabled: false },
    stroke: { curve: "smooth", width: 2.5 },
    xaxis: {
      categories: dailyOrderCount?.map((item) => format(new Date(item.period), "MMM dd")) || [],
      labels: { style: { colors: "#64748b", fontSize: "11px" } },
      axisBorder: { show: false },
      axisTicks: { show: false },
    },
    yaxis: {
      labels: { formatter: (val) => formatNumber(val), style: { colors: "#64748b", fontSize: "11px" } },
    },
    fill: {
      type: "gradient",
      gradient: { shadeIntensity: 1, opacityFrom: 0.45, opacityTo: 0.05, stops: [0, 100] },
    ...
    colors: ["#3b82f6"],
    tooltip: { y: { formatter: (val) => `${val} orders` } },
    grid: { borderColor: "rgba(59, 130, 246, 0.08)", strokeDashArray: 4 },
  };

  const dailyOrderChartSeries = [{ name: "Orders", data: dailyOrderCount?.map((item) => item.count) || [] }];
  ...
  const monthlyEarningsChartOptions = {
    ...
    dataLabels: { enabled: false },
    xaxis: {
      categories: monthlyEarnings?.map((item) => item.period) || [],
      labels: { style: { colors: "#64748b", fontSize: "11px" } },
      axisBorder: { show: false },
      axisTicks: { show: false },
    },
    ...
    colors: ["#10b981"],
    tooltip: { y: { formatter: (val) => formatCurrency(val) } },
    grid: { borderColor: "rgba(16, 185, 129, 0.08)", strokeDashArray: 4 },
  };

  const monthlyEarningsChartSeries = [{ name: "Earnings", data: monthlyEarnings?.map((item) => item.revenue) || [] }];

  const statusColors = {
    pending: "#f59e0b",
    payment_completed: "#7C3AED",
  ...
  const dailyOrdersStatusCategories = [...new Set(dailyOrdersByStatus?.map(item => item.period) || [])].sort();
  const dailyOrdersStatusLabels = [...new Set(dailyOrdersByStatus?.map(item => item.status) || [])];

  const dailyOrdersStatusSeries = dailyOrdersStatusLabels.map(status => {
    return {
      name: formatStatus(status),
      data: dailyOrdersStatusCategories.map(date => {
        const record = dailyOrdersByStatus?.find(item => item.period === date && item.status === status);
        return record ? record.count : 0;
      })
    };
  });

  const dailyOrdersStatusChartOptions = {
    chart: { type: "bar", stacked: true, height: 320, toolbar: { show: false }, fontFamily: "inherit" },
    dataLabels: { enabled: false },
    xaxis: {
      categories: dailyOrdersStatusCategories.map(date => format(new Date(date), "MMM dd")),
      labels: { style: { colors: "#64748b", fontSize: "11px" } },
      axisBorder: { show: false },
      axisTicks: { show: false },
    },
    ...
    colors: dailyOrdersStatusLabels.map(status => statusColors[status] || "#94a3b8"),
    tooltip: { y: { formatter: (val) => `${val} orders` } },
    legend: { position: "bottom", fontSize: "12px" },
    grid: { borderColor: "rgba(100, 116, 139, 0.08)", strokeDashArray: 4 },
  };

  const branchNames = branchPerformance?.details?.map(b => b.name) || [];
  
  const branchEarningsSeries = branchPerformance?.details?.map(b => b.earnings) || [];
  const branchEarningsChartOptions = {
  ...
  const branchOrdersSeries = branchPerformance?.details?.map(b => b.orders) || [];
  const branchOrdersChartOptions = {
    chart: { type: "pie", fontFamily: "inherit" },
    labels: branchNames,
    colors: ["#8b5cf6", "#ec4899", "#14b8a6", "#3b82f6", "#10b981", "#f59e0b", "#ef4444"],
    dataLabels: { enabled: false },
    legend: { position: "bottom", fontSize: "12px" },
    stroke: { show: false },
    tooltip: { y: { formatter: (val) => `${val} orders` } }
  ...
  const branchCashierPayoutsSeries = branchPerformance?.details?.map(b => b.cashierPayouts || 0) || [];
  const branchCashierPayoutsOptions = {
    chart: { type: "bar", height: 320, toolbar: { show: false }, fontFamily: "inherit" },
    ...
    colors: ["#f59e0b", "#ef4444", "#8b5cf6", "#ec4899", "#14b8a6", "#3b82f6", "#10b981"],
    legend: { show: false },
    tooltip: { y: { formatter: (val) => formatCurrency(val) } },
    grid: { borderColor: "rgba(100, 116, 139, 0.08)", strokeDashArray: 4 },
  };
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="glass-panel rounded-2xl p-5 md:p-6">
        ...
        <div className="flex flex-wrap items-end gap-4">
          ...
          {allBranches && allBranches.length > 0 && (
            <div className="flex min-w-[140px] flex-1 flex-col gap-2">
              <label className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t("BRANCH", { defaultMessage: "BRANCH" })}</label>
              <select
                value={branchId}
                onChange={(e) => setBranchId(e.target.value)}
                className="h-10 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm ring-offset-background focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2"
              >
                <option value="">{t("All_Branches", { defaultMessage: "All Branches" })}</option>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4 md:gap-5">
        <StatCard
          title={t("Vendor_Total_Earning", { defaultMessage: "Total Earning" })}
        ...
        />
        <StatCard
          title={t("Vendor_Pending_Orders", { defaultMessage: "Pending Orders" })}
          value={overview?.pendingOrders?.current || 0}
          change={overview?.pendingOrders?.change}
          icon={Users}
          gradientIndex={2}
          isCurrency={false}
        />
        <StatCard
          title={t("Vendor_Pending_Balance", { defaultMessage: "Pending Balance" })}
          value={overview?.pendingBalance?.current || 0}
          change={overview?.pendingBalance?.change}
          icon={Package}
          gradientIndex={3}
          isCurrency={false}
        />
      ...
      {/* Trends & Analysis (New Charts) */}
      <div className="mt-8 flex items-center gap-2">
        <div className="flex size-8 items-center justify-center rounded-lg bg-gradient-to-br from-violet-500/20 to-purple-600/20">
          <Activity className="size-4 text-violet-600" />
        </div>
        <h2 className="text-xl font-bold text-foreground">{t("Trends_and_Analysis", { defaultMessage: "Trends & Analysis" })}</h2>
      </div>

      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 md:gap-5">
        <GlassCard title={t("Daily_Order_Count", { defaultMessage: "Daily order count (Last 7 days)" })} icon={Activity}>
          {dailyOrderCount?.length > 0 ? (
            <Chart options={dailyOrderChartOptions} series={dailyOrderChartSeries} type="area" height={320} />
          ) : (
            <EmptyChart message={t("No_daily_order_data", { defaultMessage: "No daily order data available" })} />
          )}
        </GlassCard>
        <GlassCard title={t("Monthly_Earnings", { defaultMessage: "Monthly earnings (Last 6 months)" })} icon={BarChart3}>
          {monthlyEarnings?.length > 0 ? (
            <Chart options={monthlyEarningsChartOptions} series={monthlyEarningsChartSeries} type="bar" height={320} />
          ) : (
            <EmptyChart message={t("No_monthly_earnings_data", { defaultMessage: "No monthly earnings data available" })} />
          )}
        </GlassCard>
      </div>

      <div className="grid grid-cols-1 gap-4 md:gap-5">
        <GlassCard title={t("Daily_Orders_By_Status", { defaultMessage: "Daily orders by status (Last 30 days)" })} icon={BarChart3}>
          {dailyOrdersByStatus?.length > 0 ? (
            <Chart options={dailyOrdersStatusChartOptions} series={dailyOrdersStatusSeries} type="bar" height={320} />
          ) : (
            <EmptyChart message={t("No_daily_status_data", { defaultMessage: "No daily status data available" })} />
          )}
        </GlassCard>
      </div>

      {/* Branch Performance Section */}
      {branchPerformance && branchPerformance.overview.totalBranches > 0 && (
        <div className="mt-8 space-y-6 md:space-y-8">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-foreground">{t("Branch_Performance", { defaultMessage: "Branch Performance" })}</h2>
          </div>

          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4 md:gap-5">
            <StatCard
              title={t("Total_Branches", { defaultMessage: "Total Branches" })}
              value={branchPerformance.overview.totalBranches}
              icon={Store}
              gradientIndex={1}
              isCurrency={false}
            />
            <StatCard
              title={t("Combined_Branch_Earnings", { defaultMessage: "Combined Branch Earnings" })}
              value={branchPerformance.overview.combinedEarnings}
              icon={DollarSign}
              gradientIndex={0}
            />
            <StatCard
              title={t("Top_Performing_Branch", { defaultMessage: "Top Performing Branch" })}
              value={branchPerformance.overview.topPerforming}
              icon={Sparkles}
              gradientIndex={2}
              isCurrency={false}
            />
            <StatCard
              title={t("Average_Branch_Revenue", { defaultMessage: "Average Branch Revenue" })}
              value={branchPerformance.overview.averageRevenue}
              icon={Activity}
              gradientIndex={3}
            />
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3 md:gap-5">
            <GlassCard title={t("Branch_Earnings_Comparison", { defaultMessage: "Branch Earnings Comparison" })} icon={PieChart}>
              {branchPerformance.details?.length > 0 ? (
                <Chart options={branchEarningsChartOptions} series={branchEarningsSeries} type="pie" height={320} />
              ) : (
                <EmptyChart message={t("No_data_available", { defaultMessage: "No data available" })} />
              )}
            </GlassCard>
            
            <GlassCard title={t("Branch_Orders_Comparison", { defaultMessage: "Branch Orders Comparison" })} icon={PieChart}>
              {branchPerformance.details?.length > 0 ? (
                <Chart options={branchOrdersChartOptions} series={branchOrdersSeries} type="pie" height={320} />
              ) : (
                <EmptyChart message={t("No_data_available", { defaultMessage: "No data available" })} />
              )}
            </GlassCard>

            <GlassCard title={t("Cashier_Payouts_SAR", { defaultMessage: "Cashier Payouts (SAR)" })} icon={BarChart3}>
              {branchPerformance.details?.length > 0 ? (
                <Chart options={branchCashierPayoutsOptions} series={[{ name: "Payouts", data: branchCashierPayoutsSeries }]} type="bar" height={320} />
              ) : (
                <EmptyChart message={t("No_data_available", { defaultMessage: "No data available" })} />
              )}
            </GlassCard>
          ...
          <GlassCard title={t("Branch_Details", { defaultMessage: "Branch Details" })} icon={Store}>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-violet-100/80 dark:border-white/10 bg-violet-50/40 dark:bg-slate-900/40">
                    {[
                      t("Branch_Name", { defaultMessage: "Branch Name" }),
                      t("Status", { defaultMessage: "Status" }),
                      t("Orders", { defaultMessage: "Orders" }),
                      t("Earnings", { defaultMessage: "Earnings" }),
                      t("Revenue_Contribution", { defaultMessage: "Revenue Contribution" })
                    ].map((col) => (
                ...
                <tbody>
                  {branchPerformance.details.map((branch, idx) => (
                    <tr key={idx} className="border-b border-violet-50/80 dark:border-white/10 transition-colors hover:bg-white/50 dark:hover:bg-slate-800/50">
                      ...
                      <td className="px-4 py-3 text-sm">{formatNumber(branch.orders)}</td>
                      <td className="px-4 py-3 text-sm font-semibold">{formatCurrency(branch.earnings)}</td>
                      <td className="px-4 py-3 text-sm text-muted-foreground">
                        <div className="flex items-center gap-2">
                          <div className="h-1.5 w-16 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                            <div 
                              className="h-full rounded-full bg-violet-500" 
                              style={{ width: `${branch.revenueContribution}%` }}
                            />
                          </div>
                          <span>{branch.revenueContribution}%</span>
                        </div>
                      </td>
                    </tr>
                  ))}

...
export default function RegisterVendor() {
  ...
  const onSubmit = async (data) => {
    setIsSuccess(false);
    try {
      // Blade template used password & confirm_password but let's send standard mapping
      const payload = {
        name: data.name,
        email: data.email,
      ...
      <div className="w-full max-w-md mt-12 mb-8 z-10 relative">
        <div className="text-center mb-8">
          ...
          <p className="text-[var(--t-text-muted)]">
            {t("vendor_register_lead") === "messages.vendor_register_lead" ? "Join us as a vendor and start selling today." : t("vendor_register_lead")}
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div>
              <label className="block text-sm font-medium text-[var(--t-text)] mb-1">
                {t("vendor_name") === "messages.vendor_name" ? "Vendor Name" : t("vendor_name")} *
              </label>
              <input
                type="text"
                ...
                placeholder={t("vendor_name") === "messages.vendor_name" ? "Vendor Name" : t("vendor_name")}
              />
              {errors.name && (

...
export default function RegisterCustomer() {
  ...
  const onSubmit = async (data) => {
    setIsSuccess(false);
    try {
      // The backend expects "password_confirmation" but let's be sure it maps properly.
      const payload = {
        name: data.name,
        email: data.email,
      ...
      <div className="w-full max-w-md mt-12 mb-8 z-10 relative">
        <div className="text-center mb-8">
          ...
          <p className="text-[var(--t-text-muted)]">
            {t("create_account_shopping") === "messages.create_account_shopping" ? "Create an account to start shopping" : t("create_account_shopping")}
          </p>
        </div>

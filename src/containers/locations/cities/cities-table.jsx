...
import { dummyCities } from "./cities-dummy-data";
...
export default function CitiesTable() {
  ...
  const fetchTableData = async (page, pageSize, search = "") => {
    ...
    try {
      ...
      setTableData(response.data.data?.cities || []);
  ...
  const handleSave = async (data, id) => {
    try {
      let response;
      if (id) {
        response = await axiosInstance.patch(`/locations/cities/${id}`, data);
      } else {
        response = await axiosInstance.post("/locations/cities", data);
      }
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await axiosInstance.delete(`/locations/cities/${itemToDelete.id}`);
  ...
  return (
    <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl border border-white/50 dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-2xl p-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 my-4">
        ...
        {hasPermissionSync(Permissions.ADD_CITIES) && (
          <Button
            variant="secondary"
            onClick={handleAdd}
      ...
      <TableToolbar
        ...
        rightSlot={
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{total} cities found</span>
          </div>
        }
      />

      <DataTable
        columns={citiesColumns(handleEdit, handleDelete)}
        data={tableData}
        page={page}
        setPage={setPage}
        pageSize={pageSize}

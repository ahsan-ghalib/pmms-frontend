...
import { statesColumns } from "./states-columns";
...
export default function StatesTable() {
  ...
  const fetchTableData = async (page, pageSize, search = "") => {
    ...
    try {
      ...
      setTableData(response.data.data?.states || []);
  ...
  const handleSave = async (data, id) => {
    try {
      let response;
      if (id) {
        response = await axiosInstance.patch(`/locations/states/${id}`, data);
      } else {
        response = await axiosInstance.post("/locations/states", data);
      }
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await axiosInstance.delete(`/locations/states/${itemToDelete.id}`);
  ...
  return (
    <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl border border-white/50 dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-2xl p-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 my-4">
        ...
        {hasPermissionSync(Permissions.ADD_STATES) && (
          <Button
            variant="secondary"
            onClick={handleAdd}
      ...
      <TableToolbar
        ...
        rightSlot={
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{total} states found</span>
          </div>
        }
      />

      <DataTable
        columns={statesColumns(handleEdit, handleDelete)}
        data={tableData}
        page={page}
        setPage={setPage}
        pageSize={pageSize}
